geocode_by_postal_code <- function(
  postal_code,
  method = "qualocep",
  ...
  if (method %in% c("osm", "google")) {
    geocode_by_postal_code.tidygeocoder(
      postal_code, method, limit, suffix
    )
  } else if (method == "qualocep") {
    geocode_by_postal_code.qualocep(postal_code, limit)
  }
}

geocode_by_postal_code.tidygeocoder <- function( #nolint
    postal_code, #nolint
    method = "osm",
    limit = 10,
    suffix = ", Brazil"
...
geocode_by_postal_code.qualocep <- function( #nolint
    postal_code, #nolint
    limit = 10
  ) {
  require_pkg("osfr")
  ...
  qualocep_data <- qualocep_download()
