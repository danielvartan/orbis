geocode_by_address <- function(
  ...
  if (method %in% c("osm", "google")) {
    geocode_by_address.tidygeocoder(address, method, limit)
  } else if (method == "qualocep") {
    geocode_by_address.qualocep(limit, ...)
  }
}

geocode_by_address.tidygeocoder <- function( #nolint
    address, #nolint
    method = "osm",
    limit = 10
  ) {
...
geocode_by_address.qualocep <- function( #nolint
    limit = 10, #nolint
    mean_values = TRUE,
    ...
  ) {
